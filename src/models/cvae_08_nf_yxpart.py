...
class CVAE8(nn.Module):
	...
	def __init__(self, model_config, va):
		...
		self.decoder = Decoder(x_dim, z_dim, y_dim, self.hidden_sizes, n_h, va.dim_config)
	...
	def reparametrize(self, mu, logvar):
		...
		log_q_z0 = -(torch.log(std) + 0.5*(eps**2) + 0.5*torch.log(torch.tensor(2*math.pi)))
	...
	def forward(self, x, y, y_x_partial):
		...
		kl_prior = -0.5*(torch.log(torch.tensor(2 * math.pi)) + zk**2)

		kl_divergence = kl_posterior - kl_prior

		# generative phase
		recon_x = self.decoder(zk, y, y_x_partial)
...
class Decoder(nn.Module):
	...
	def __init__(self, x_dim, z_dim, y_dim, hidden_sizes, n_h, dim_config):
		super().__init__()
	...
	def forward(self, z, y, y_x_partial):
		...
		if self.dim_config is not None:
			sigmoid_mask = self.dim_config.get("x_output_masks").get("sigmoid")
			x[:, sigmoid_mask] = nn.Sigmoid()(x[:, sigmoid_mask])

			softmax_masks = self.dim_config.get("x_output_masks").get("softmax")
			for softmax_mask in softmax_masks:
				x[:, softmax_mask] = nn.Softmax(dim=1)(x[:, softmax_mask])
		else:
			x = nn.Sigmoid()(x)
...
class IAFLayer(nn.Module):
	...
	def __init__(self, z_dim, h_context_dim):
		...
		self.sigmoid_arg_bias = nn.Parameter(torch.ones(z_dim) * 1.5)
