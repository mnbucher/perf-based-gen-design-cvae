...
class GCNN(torch.nn.Module):
	...
	def __init__(self, model_config, va):
		...
		for i in range(self.gnn_n_h):
			inp_dim = 2 if i == 0 else self.gnn_h_dim
			out_dim = self.gnn_h_dim
			self.gnn.add_module(name=f"gnn5_gnn_gcn_{i}", module=GCNBlock(va, i, inp_dim, out_dim))
	...
	def forward(self, data, x_10d):
		...
		x_with_edges = x, data.edge_index, edge_weights, None

		# gnn forward pass
		x, _, _, _ = self.gnn(x_with_edges)

		# global pooling into single latent embedding
		x = self.aggregate_node_features(x, data.batch)

		# mlp to perform regression
		if self.concat_x_with_z_g_for_mlp_head:
			x = torch.cat([x, x_10d], dim=1)

		x, x_embedd = self.mlp(x)
		x = nn.Sigmoid()(x)
...
class GCNBlock(nn.Module):
	...
	def __init__(self, va, idx, inp_dim, out_dim):
		super().__init__()
		...
		self.conv = GCNConv(inp_dim, out_dim, improved=True)
		self.activ = nn.ReLU()
		self.bn = nn.BatchNorm1d(out_dim)

	def forward(self, x_with_edges):

		x, edge_indices, edge_weights, _ = x_with_edges
		last_hidd = x

		x1 = self.conv(x, edge_indices, edge_weights)
		x1 = self.activ(x1)
		x1 = self.bn(x1)

		x = x1

		x_with_edges = x, edge_indices, edge_weights, last_hidd
...
class MLPBlock(nn.Module):
	...
	def forward(self, x):
		...
		if self.is_last_layer and self.do_return_embedding:
			return x3, x
